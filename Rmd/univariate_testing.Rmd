...
For categorical variables, I set all values with a frequency less than 1% to *rare*. This mostly affected the variable capturing county-of-birth information and detailed household statistics. 

## AUC measurements for each variable

I tested the predictive capability of each variable using a random forest. A separate random forest was grown for each variable, and its performance was measured using an out-of-bounds AUC measurement. We could attempt to recover some of the information in these variables by mapping countries to regions and reducing the number of household categories.
